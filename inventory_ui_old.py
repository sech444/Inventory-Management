...
class EnhancedInventory:
    ...
    def sell_item(self, sku, qty, sale_price=None, customer=""):
        if sku not in self._items:
        ...
        cursor = conn.cursor()
        cursor.execute('''
            INSERT INTO sales (sku, quantity, sale_price, profit, date, customer)
            VALUES (?, ?, ?, ?, ?, ?)
        ...
        if self.current_user:
            self.db.log_action(self.current_user, "SALE", 
                             f"Sold {qty} units of {sku} for ${sale_price or self._items[sku].unit_price} each")
    ...
    def get_sales_data(self, days=30):
        ...
        cursor.execute('''
            SELECT * FROM sales WHERE date >= ? ORDER BY date DESC
        ''', (start_date,))
        
        sales = []
        for row in cursor.fetchall():
            sales.append(Sale(row[0], row[1], row[2], row[3], row[4], row[5], row[6]))
...
class EnhancedInventoryApp:
    ...
    def show_login(self):
        ...
        username_entry = ttk.Entry(login_window, width=20, font=('Helvetica', 10))
        username_entry.pack(pady=5)
        
        # Password
        ttk.Label(login_window, text="Password:").pack()
        password_entry = ttk.Entry(login_window, width=20, font=('Helvetica', 10), show="*")
        ...
        ttk.Button(login_window, text="Login", command=do_login).pack(pady=20)
    ...
    #     # Sale form
    #     sale_form = ttk.LabelFrame(main_container, text=" Record Sale ", padding=15)
    ...
    #     self.sale_customer = ttk.Entry(form_grid, width=20)
    #     self.sale_customer.grid(row=1, column=1, columnspan=2, sticky='ew', padx=(0,10), pady=(10,0))
        
    #     ttk.Button(form_grid, text="💰 Record Sale", command=self._record_sale).grid(row=1, column=4, columnspan=2, pady=(10,0))
        
    #     # Available stock info
    #     self.stock_info = ttk.Label(form_grid, text="", foreground='blue')
    #     self.stock_info.grid(row=2, column=0, columnspan=6, pady=(10,0))
        
    #     # Sales history
    #     history_frame = ttk.LabelFrame(main_container, text=" Sales History ", padding=10)
    ...
    #     self.sales_period.bind('<<ComboboxSelected>>', self._refresh_sales_history)
        
    #     ttk.Button(filter_frame, text="🔄 Refresh", command=self._refresh_sales_history).pack(side='left')
        
    #     # Sales tree
    #     sales_columns = ('Date', 'SKU', 'Quantity', 'Sale Price', 'Total Sale', 'Profit', 'Customer')
    #     self.sales_tree = ttk.Treeview(history_frame, columns=sales_columns, show='headings', height=12)
        
    #     for col in sales_columns:
    #         self.sales_tree.heading(col, text=col)
    #         width = 120 if col in ['Date', 'Sale Price', 'Total Sale', 'Profit'] else 100
    #         if col == 'Customer':
    #             width = 150
    ...
    def _create_sales_tab(self):
        ...
        # FIXED: Change filter_frame to use grid instead of pack
        filter_frame = ttk.Frame(history_frame)
    ...
    def _create_calculator_tab(self):
        ...
        calc_container.pack(expand=True)
        ...
        btn_cfg = {"font": ("Helvetica", 14), "width": 5, "takefocus": False,
                   "bg": "#4a7c59", "fg": "white", "activebackground": "#5d8b6a"}
        ...
        # Special buttons
        tk.Button(calc_container, text="C", command=self._calc_clear, 
                 bg="#c62828", fg="white", font=("Helvetica", 14), 
                 activebackground="#d32f2f").grid(row=5, column=0, columnspan=2, padx=2, pady=2, sticky="nsew")
        tk.Button(calc_container, text="←", command=self._calc_backspace, 
                 bg="#f57c00", fg="white", font=("Helvetica", 14),
                 activebackground="#ff9800").grid(row=5, column=2, columnspan=2, padx=2, pady=2, sticky="nsew")

        for i in range(6):
            calc_container.rowconfigure(i, weight=1)
        for i in range(4):
            calc_container.columnconfigure(i, weight=1)
    ...
    def _record_sale(self):
        ...
        try:
            ...
            profit = self.inventory.sell_item(sku, qty, price, customer)
    ...
    def _refresh_sales_history(self, event=None):
        ...
        sales = self.inventory.get_sales_data(days)
    ...
    def _show_sales_chart(self):
        ...
        sales = self.inventory.get_sales_data(30)
...
"""
    ...
    def _create_comprehensive_report(self):
        ...
        analytics = self.inventory.get_analytics()
        sales_30d = self.inventory.get_sales_data(30)
